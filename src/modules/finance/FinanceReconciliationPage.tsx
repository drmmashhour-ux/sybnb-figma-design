...
const copy = {
  ar: {
    ...
    srNote: 'رحلات SYBNB Ride: {count} رحلة مكتملة بقيمة أجرة إجمالية {fare} — هذه أرباح السائقين، والمنصة لا تُحصّل عمولة من رحلات SYBNB Ride حالياً.',
    ...
    whatIfNote: 'أدخل افتراضاتك الخاصة — هذه ليست بيانات حقيقية، لكن الحساب يستخدم نفس صيغة عمولة SYBNB الفعلية (تنظيف ٥٪ + عمولة استضافة ١٣٪ من الإيجار الصافي). لا توجد ضريبة منصّة مخترعة؛ ضرائب كيبك الحقيقية (٣٫٥٪ إقامة + GST + QST) تُحسب بشكل منفصل عند تفعيلها.',
    ...
    srRidesPerMonth: 'رحلات SYBNB Ride شهرياً',
    ...
    srDriverVolumeNote: 'أجرة رحلات SYBNB Ride الشهرية المفترضة: {fare} — أرباح سائقين، ليست إيراد منصة (لا عمولة على SYBNB Ride حالياً).',
  },
  en: {
    back: 'Back to admin',
    ...
    srNote: '{count} completed SYBNB Ride trips worth {fare} in total fares — that\'s driver earnings; the platform currently collects no commission on SYBNB Ride trips.',
    ...
    whatIfNote: 'Enter your own assumptions — this is not real data, but the math uses the real SYBNB commission formula (5% cleaning + 13% host commission on net rent). There is no invented platform tax; real Québec taxes (3.5% lodging + GST + QST) are calculated separately once activated.',
    ...
    srRidesPerMonth: 'SYBNB Ride trips per month',
    ...
    srDriverVolumeNote: 'Assumed monthly SYBNB Ride fare volume: {fare} — driver earnings, not platform revenue (no SYBNB Ride commission today).',
  },
}
