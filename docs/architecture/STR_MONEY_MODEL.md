...
| `platformCommissionMinor` (returned as `adminCommissionMinor`) | `round(nightlySubtotalMinor × 13%)` — SYBNB's commission, computed on the accommodation portion only, never on the cleaning fee. | `finance-ledger.mjs`'s `bookingFinanceSplit` | — |
